(function () {
  ...
  // ✅ Supabase client for auth (and anything else)
  const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON);

  // ✅ Helper: require login on protected pages
  ...
  async function getOrCreateSupplierId(name) {
    const res1 = await fetch(
      `${SUPABASE_URL}/rest/v1/suppliers?select=id&name=eq.${encodeURIComponent(name)}`,
      {
        headers: {
          apikey: SUPABASE_ANON,
          Authorization: `Bearer ${SUPABASE_ANON}`,
  ...
  async function updateNcr(id, patch) {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/ncrs?id=eq.${encodeURIComponent(id)}`,
      {
        method: "PATCH",
        headers: {
          apikey: SUPABASE_ANON,
  ...
  async function getNcrById(id) {
    const params = new URLSearchParams({
      select:
        "id,ncr_no,status,product_no,so_no,date_raised,date_closed,wip,defect_desc,is_nc,qty_supplied,qty_defective,rep_name,supplier_id",
      limit: "1"
    });
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/ncrs?id=eq.${encodeURIComponent(
        id
      )}&${params}`,
      {
        headers: {
  ...
  async function closeNcr(ncrId, closedBy) {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/ncrs?id=eq.${encodeURIComponent(ncrId)}`,
      {
        method: "PATCH",
        headers: {
          apikey: SUPABASE_ANON,
