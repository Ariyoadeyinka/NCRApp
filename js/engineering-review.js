(function () {
    ...
    async function loadNcr() {
        ...
        try {
            ...
            if (currentStage !== "engineering" && currentStage !== "operations") {
                console.warn("NCR current_stage is neither engineering nor operations:", n.current_stage);
            }
            ...
            setText(
                q("#reviewStatus"),
                (n.status || "open").charAt(0).toUpperCase() + (n.status || "open").slice(1)
            );
            setText(
                q("#reviewStage"),
    ...
    async function loadEngineering() {
        ...
        try {
            ...
            if (q("#engRevisionDate"))
                q("#engRevisionDate").value = row.revision_date ? fmtDate(row.revision_date) : "";
            if (q("#engSignatureDate"))
                q("#engSignatureDate").value = row.signature_date ? fmtDate(row.signature_date) : "";
    ...
    async function closeNcrInEngineering() {
        ...
        const { data, error } = await client
            ...
            .eq("id", ncrId)
            .select(
                "id,status,date_closed,closed_by,current_stage,whose_turn_dept,next_up_dept"
            )
            .single();

        if (error) {
            console.error("closeNcrInEngineering error:", error);
            throw error;
        }
        return data;
    }
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        q("#btnGoReview")?.addEventListener("click", () => {
            populateReviewFromForm();
            switchToReviewStep();
        });

        q("#btnBackToEdit")?.addEventListener("click", () => {
            switchToEditStep();
        });
            ...
            if (currentStage === "operations") {
                ...
                btnForward.addEventListener("click", async () => {
                    try {
                        ...
                        console.error(e);
                        showFailModal("Save Failed", e.message || "Failed to save engineering changes.");
                    } finally {
                ...
                btnForward.addEventListener("click", async () => {
                    try {
                        ...
                        const ncrId = Number(readQueryId());
                        const { error } = await window.NCR.auth.client.rpc("forward_to_operations", {
                            p_ncr_id: ncrId
                        });
                        if (error) throw error;
