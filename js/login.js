...
(function () {
    ...
    // --- helper: load roles from Supabase + store to localStorage ---
    async function loadAndStoreRoles(profileId) {
        try {
            // profile_roles(profile_id, role_id) → roles(code)
            ...
            // store list of all roles
            localStorage.setItem("cf_roles", JSON.stringify(codes));

            // also store a primary role for convenience (admin > engineering > quality > operations)
    ...
    // --- Global auth helper used by all pages ---
    ...
    // --- Page-level behaviour (login page + protected-page gating) ---
    document.addEventListener("DOMContentLoaded", function () {
        const form = document.querySelector(".auth-form"); // present only on login page
        const params = new URLSearchParams(window.location.search);
        const hasRedirectParam = params.has("redirect");

        // Wire up SIGN OUT link on any page that has it
        ...
        // If this is NOT the login page, treat it as a protected page
        if (!form) {
            (async () => {
                const session = await window.NCR.auth.requireLogin();
                if (!session) return;

                // make sure roles are loaded for this session (in case of refresh)
                await loadAndStoreRoles(session.user.id);
            })();
            return;
        }

        // --- We *are* on the login page from here down ---

        // Auto-bounce if already logged in + redirect param
        if (hasRedirectParam) {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                // ✅ load roles for this user and store into localStorage
                if (data && data.user) {
